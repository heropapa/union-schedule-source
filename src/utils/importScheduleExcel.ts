...
async function parseAdmin(
  ...
  weekDates: string[],
  workers: Worker[],
  allowedCampNames?: string[],
): Promise<ScheduleImportResult> {
  const rows = parseAdminExcel(buffer);
  const result = matchImportRows(rows, workers, campId);

  // 안전장치 1: 허용 캠프(현재 캠프 + 계약라우트에 등록된 캠프명) 밖의 캠프명이면 그 행만 건너뜀.
  // 파일 전체가 허용 밖 캠프뿐이면 사실상 전체 거부와 동일한 결과.
  const allowed = new Set([campName, ...(allowedCampNames ?? [])].filter(Boolean));
...
export async function importScheduleExcel(
  buffer: ArrayBuffer,
  format: '일반' | '어드민',
  ctx: { campId: string; campName: string; weekDates: string[]; workers: Worker[]; allowedCampNames?: string[] },
): Promise<ScheduleImportResult> {
  if (format === '어드민') return parseAdmin(buffer, ctx.campId, ctx.campName, ctx.weekDates, ctx.workers, ctx.allowedCampNames);
